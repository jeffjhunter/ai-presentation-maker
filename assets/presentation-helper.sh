...
# Resolve the agent workspace: $OPENCLAW_WORKSPACE -> agents.defaults.workspace in
# ~/.openclaw/openclaw.json -> current default ~/.openclaw/workspace
if [[ -n "${OPENCLAW_WORKSPACE:-}" ]]; then
  _WS="${OPENCLAW_WORKSPACE}"
elif [[ -f "${HOME}/.openclaw/openclaw.json" ]] && command -v jq &>/dev/null; then
  _WS=$(jq -r '.agents.defaults.workspace // empty' "${HOME}/.openclaw/openclaw.json" 2>/dev/null)
  _WS="${_WS:-${HOME}/.openclaw/workspace}"
else
  _WS="${HOME}/.openclaw/workspace"
fi
PRES_DIR="${_WS}/presentations"
...
cmd_save_meta() {
  local tmp_dir="${PRES_DIR}/.tmp"
  mkdir -p "$tmp_dir"; chmod 700 "$tmp_dir"
  local tmp; tmp=$(mktemp "${tmp_dir}/pres_meta.XXXXXX") || { echo "ERROR: mktemp failed" >&2; return 1; }
  chmod 600 "$tmp"
  cat > "$tmp"
